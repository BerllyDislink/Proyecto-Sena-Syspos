...
function LoginView({ onForgot }) {
  ...
  const inp = (f) =>
    `w-full bg-primary-900 border rounded-xl pl-11 pr-4 py-3 text-sm text-slate-100 placeholder-slate-600 outline-none focus:ring-2 transition-all ${
      errors[f] ? 'border-red-500 focus:ring-red-500' : 'border-primary-700 focus:ring-primary-500'
    }`

  return (
    <div className="min-h-screen bg-gradient-to-br from-secondary-950 via-secondary-900 to-primary-900 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative">
        <div className="bg-primary-900 border border-primary-800 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden">
          <div className="h-1 w-full bg-gradient-to-r from-primary-600 via-secondary-600 to-primary-600" />
          <div className="px-8 py-10">

            {/* Logo */}
            <div className="flex flex-col items-center mb-8">
              <div className="w-14 h-14 rounded-2xl bg-primary-600 flex items-center justify-center font-bold text-white text-2xl shadow-lg shadow-primary-500/30 mb-4">S</div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button type="submit" disabled={loading}
                className="w-full py-3 rounded-xl bg-primary-600 hover:bg-primary-500 disabled:opacity-60 text-white font-semibold text-sm transition-all shadow-lg shadow-primary-500/20 flex items-center justify-center gap-2">
                {loading
  ...
  if (sent) return (
    <div className="min-h-screen bg-gradient-to-br from-secondary-950 via-secondary-900 to-primary-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-primary-900 border border-primary-800 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="px-8 py-10 flex flex-col items-center text-center">
            ...
            <p className="text-primary-400 font-medium text-sm mb-5">{email}</p>
            <p className="text-slate-500 text-xs mb-8 leading-relaxed">
              Recibirás un enlace de Supabase para restablecer tu contraseña. El enlace expira en 1 hora.
            </p>
            <button onClick={onBack}
              className="w-full py-3 rounded-xl bg-primary-600 hover:bg-primary-500 text-white font-semibold text-sm transition-all">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email}
                    ...
                    className={`w-full bg-primary-900 border rounded-xl pl-11 pr-4 py-3 text-sm text-slate-100 placeholder-slate-600 outline-none focus:ring-2 transition-all ${error ? 'border-red-500 focus:ring-red-500' : 'border-primary-700 focus:ring-primary-500'}`} />
                </div>
                {error && <p className="text-red-400 text-xs mt-1.5">{error}</p>}
              </div>
              <button type="submit" disabled={loading}
                className="w-full py-3 rounded-xl bg-primary-600 hover:bg-primary-500 disabled:opacity-60 text-white font-semibold text-sm transition-all flex items-center justify-center gap-2">
                {loading
